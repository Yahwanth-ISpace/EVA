...
7. **Turn loop:** User speaks → we buffer audio → on silence (or max buffer), we transcribe (ElevenLabs/Whisper) → send transcript + current `extractedData` + `patientInfo` to **AI** (`getNextConversationTurn`) → AI returns `nextMessage`, `extractedUpdates`, `endCall` → we merge updates, validate/normalize (dollars → **$**, percent → **%**), speak EVA’s reply.
